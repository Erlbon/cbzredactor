...
PyQt6 Windows desktop tool for viewing and editing the `ComicInfo.xml` metadata inside CBZ comic archives (title, series, credits, publisher, classification) without touching page images. Creates `ComicInfo.xml` on save if missing. Also reads CBT/CB7 and offers a conversion prompt. Part of the Redactor family; shared code lives in [redactor_common](https://github.com/Erlbon/redactor_common), pinned by tag in `requirements.txt`.

## Commands
- Run: `pip install -r requirements.txt` then `python main.py`
- Test: `pip install pytest` then `pytest` (plain pytest, `test_*.py` at the repo root; `core/` needs no Qt)
- Build exe: `build_exe.bat` (`cbzredactor.spec`)
- Bump version: `python bump_version.py`

## Layout
`core/`, `gui/`, `main.py`, `CHANGELOG.md`, `ABOUT.md`, `CREDITS.md`.

## Domain notes
- ComicInfo.xml follows the ComicRack/Anansi Project schema. Comma-separated convention for Genre, Characters, Teams, Locations, StoryArc and credits was checked against ComicTagger's source (`comicapi/tags/comicrack.py`).
- Lookups: Comic Vine (needs a free API key; candidates are ranked, runner-ups shown), Bedetheque, GCD.
- Image resizing is done in-process, without ImageMagick.
